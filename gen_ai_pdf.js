...
const outPath = path.join(process.env.USERPROFILE || process.env.HOME, 'Desktop', 'AI科技早报_2026年3月25日.pdf');
...
doc.font(FONT).fontSize(22).fillColor('#FFFFFF')
  .text('AI 科技早报', 40, 58, { width: W, align: 'center' });
doc.font(FONT).fontSize(10).fillColor('#B0D4FF')
  .text('2026年3月25日 星期三  |  人工智能 × 大模型 × 科技趋势', 40, 106, { width: W, align: 'center' });

// ===== METRICS =====
const metricsY = 136;
doc.rect(40, metricsY, W, 56).fill(LIGHT_BLUE);

// Metric boxes
const boxW = W / 3;
const metrics = [
  { val: '140万亿+', label: '日均词元调用量' },
...
metrics.forEach((m, i) => {
  const x = 40 + i * boxW;
  doc.font(FONT).fontSize(18).fillColor(MID_BLUE)
    .text(m.val, x, metricsY + 8, { width: boxW, align: 'center' });
  doc.font(FONT).fontSize(8.5).fillColor(GRAY)
    .text(m.label, x, metricsY + 30, { width: boxW, align: 'center' });
});
...
function section(title, color) {
  ...
  doc.font(FONT).fontSize(11).fillColor('#FFFFFF')
    .text(title, 50, y + 5, { width: W - 20 });
  y += 28;
}

function newsItem(num, title, body, source) {
  if (y > 740) { doc.addPage(); y = 60; }
  doc.font(FONT).fontSize(10).fillColor(MID_BLUE)
    .text(`【${num}】`, 44, y);
  doc.font(FONT).fontSize(10.5).fillColor(DARK_BLUE)
    .text(title, 66, y, { width: W - 26 });
  y += 16;
  doc.font(FONT).fontSize(9).fillColor(GRAY)
    .text(body, 44, y, { width: W - 8, lineGap: 2, align: 'justify' });
  y += doc.heightOfString(body, { width: W - 8, align: 'justify' }) + 6;
  if (source) {
    doc.font(FONT).fontSize(7.5).fillColor('#999999')
      .text(source, 44, y - 4);
    y += 4;
  }
  y += 4;
...
function trendItem(title, body, color) {
  if (y > 740) { doc.addPage(); y = 60; }
  doc.rect(40, y, 4, 20).fill(color);
  doc.font(FONT).fontSize(10.5).fillColor(color)
    .text(title, 50, y + 1, { width: W - 16 });
  y += 18;
  doc.font(FONT).fontSize(9).fillColor(GRAY)
    .text(body, 50, y, { width: W - 16, lineGap: 2, align: 'justify' });
  y += doc.heightOfString(body, { width: W - 16, align: 'justify' }) + 8;
}
...
newsItem(1, 'OpenAI 完成新 AI 模型初步开发，Altman 战略重心转向基础设施',
  '据报道，OpenAI 已完成新 AI 模型的初步开发。Sam Altman 将不再直接负责 OpenAI 安全团队，工作重心转向数据中心建设、募资及供应链管理，战略布局从模型研发向基础设施倾斜。',
  '来源：钛媒体');
...
doc.font(FONT).fontSize(7.5).fillColor('#999999')
  .text('AI 科技早报  |  2026年3月25日  |  仅供参考，不构成投资建议', 40, y + 10, { width: W, align: 'center' });

doc.end();

out.on('finish', () => {
  console.log('PDF_SAVED:' + outPath);
});
